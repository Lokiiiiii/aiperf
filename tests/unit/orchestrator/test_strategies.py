...
class TestFixedTrialsStrategy:
    ...
    def test_should_continue_with_no_results(self):
        """Test should_continue returns True when no results yet."""
    ...
    def test_should_continue_with_partial_results(self):
        """Test should_continue returns True when results < num_trials."""
        strategy = FixedTrialsStrategy(num_trials=3)
        results = [
            RunResult(
                label="run_0001",
    ...
    def test_should_continue_with_complete_results(self):
        """Test should_continue returns False when results == num_trials."""
    ...
    def test_get_run_label_zero_padding(self):
        """Test get_run_label returns zero-padded labels."""
        strategy = FixedTrialsStrategy(num_trials=100)
    ...
    def test_get_run_label_adjusts_padding_for_large_trials(self):
        """Test get_run_label adjusts padding for large num_trials."""
        strategy = FixedTrialsStrategy(num_trials=1000)
        
        assert strategy.get_run_label(0) == "run_0001"
        assert strategy.get_run_label(999) == "run_1000"
    ...
    def test_get_cooldown_seconds_default(self):
        """Test get_cooldown_seconds returns default value."""
        strategy = FixedTrialsStrategy(num_trials=3)
    ...
    def test_get_next_config_returns_base_config_after_first_run(self):
        ...
        strategy = FixedTrialsStrategy(num_trials=3, auto_set_seed=True, disable_warmup_after_first=True)
