...
class TestMultiRunConfidence:
    ...
    async def test_multi_run_basic(
        self, cli: AIPerfCLI, aiperf_mock_server: AIPerfMockServer, temp_output_dir: Path
    ):
        ...
        with open(agg_json) as f:
            ...
            throughput_metrics = [k for k in metrics.keys() if "throughput" in k.lower()]
            assert len(throughput_metrics) > 0, "Should have throughput metrics"
            
            sample_metric = metrics[throughput_metrics[0]]
            required_fields = ["mean", "std", "min", "max", "cv", "se", "ci_low", "ci_high", "t_critical", "unit"]
    ...
    async def test_backward_compatibility_single_run(
        ...
        assert not profile_runs_dir.exists(), "profile_runs should not exist for single run"
    ...
    async def test_aggregate_csv_format(
        ...
        # Check header
        header = lines[0]
        required_columns = ["metric", "mean", "std", "min", "max", "cv", "se", "ci_low", "ci_high", "t_critical", "unit"]
    ...
    async def test_multi_run_insufficient_successful_runs(
        ...
        # Command should fail
        assert result.exit_code != 0, "Should exit with non-zero code when all runs fail"
        
        # Verify no aggregate directory was created (or it's empty)
        aggregate_dir = temp_output_dir / "aggregate"
        if aggregate_dir.exists():
            agg_json = aggregate_dir / "profile_export_aiperf_aggregate.json"
            assert not agg_json.exists(), "Aggregate JSON should not exist with insufficient successful runs"
    ...
    async def test_multi_run_all_runs_fail(
        ...
        if aggregate_dir.exists():
            agg_json = aggregate_dir / "profile_export_aiperf_aggregate.json"
            assert not agg_json.exists(), "Aggregate JSON should not exist when all runs fail"
        ...
        if aggregate_dir.exists():
            ...
            if agg_json.exists():
                with open(agg_json) as f:
                    ...
                    assert num_successful >= 2, "Aggregate requires at least 2 successful runs"
                    
                    # Verify metrics were computed
                    assert len(agg_data["metrics"]) > 0, "Should have aggregated metrics"
    ...
    async def test_multi_run_invalid_num_profile_runs(
        ...
        assert "num-profile-runs" in output.lower() or "num_profile_runs" in output.lower()
